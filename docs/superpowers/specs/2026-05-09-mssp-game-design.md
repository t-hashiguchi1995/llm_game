...
| 2 | 立ち絵差分数・感情パターン数 | 高 | `characters.json` 確定に必要 |
| 3 | 背景画像の枚数・シーン数 | 高 | `scenarios.json` 確定に必要 |
| 4 | エンディングCGの有無 | 中 | `endings.json` の `cg` フィールド |
| 5 | BGM・SE実装 | 低 | Phase 1 スコープ外 |
| 6 | GitHub リポジトリ名（Vite `base` 設定に影響） | 高 | デプロイ前に確定 |
| 7 | LOGボタン（会話履歴表示）の実装 | 低 | UIには配置するが Phase 1 は省略可 |
