		...
		(commandId: string) => {
			...
			const chainParams = chain.reduce(
				(acc, evt) => ({
					sweetness: Math.min(
						100,
						Math.max(0, acc.sweetness + evt.parameter_delta.sweetness),
					),
					curiosity: Math.min(
						100,
						Math.max(0, acc.curiosity + evt.parameter_delta.curiosity),
					),
					trust: Math.min(
						100,
						Math.max(0, acc.trust + evt.parameter_delta.trust),
					),
				}),
				state.params,
			);
